...
// static void showBits(Bits bits, char *item) {
// 	char buf[MAXBITS+1];
// 	bitsString(bits, buf);
// 	printf("%s %s\n", buf, item);
// }
...
PageID part_addToRelation(Reln r, Tuple t)
{
	Bits h, p;
	h = tupleHash(r,t);
	printf("hash = %d\n",h);
...
void insert_page_data(Page p, Reln r) {
	if (pageNTuples(p) == 0) {
		return;
	}
	Tuple t = pageData(p);
	...
	while (1) {
		part_addToRelation(r,t);
		if (tup_no_on_page == pageNTuples(p) - 1) {
			break;
		}
		else {
			tup_no_on_page ++;
...
{
	PageID pid = part_addToRelation(r, t);
	// if can't insert successfully, return NO_PAGE
	if (pid == NO_PAGE) return pid;

	r->ntups ++;
	// if no need to split, return pid
	Count c = 1024/(10*r->nattrs);
	if (r->ntups % c != 0) {
		return pid;
	}

// now split the page where sp-pointer is
	r->npages ++; 
	putPage(r->data, r->sp + (1 << r->depth), newPage());
	r->sp ++;

	Page temp_p = NULL;
	PageID temp_id = NO_PAGE;
	FILE *f = r->data;
	do {
		if (f == r->data) {
			temp_id = r->sp - 1;
			...
			temp_id = pageOvflow(temp_p);
		}
		temp_p = getPage(f, temp_id);
		Page blank = newPage();
		pageSetOvflow(blank, pageOvflow(temp_p));
		putPage(f, temp_id, blank);
		insert_page_data(temp_p, r);
		f = r->ovflow;

	} while (pageOvflow(temp_p) != NO_PAGE);
	if (r->sp == 1 << r->depth) {
		r->sp = 0;
		r->depth++;
	}
