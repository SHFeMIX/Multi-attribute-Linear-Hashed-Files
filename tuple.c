...
Bits tupleHash(Reln r, Tuple t)
{
	...
	char **vals = calloc(nvals*sizeof(char *) , 1);
	assert(vals != NULL);
	tupleVals(t, vals);

	Bits attr_hash[nvals]; // hash for each attribute
	...
	Bits hash = 0; // hash of the tuple
	ChVecItem *cv = chvec(r);
	for (int i = 0; i < MAXBITS; i++) {
		Bits hash_value = attr_hash[cv[i].att];
		int pos = cv[i].bit;
		if (bitIsSet(hash_value, pos) == 1) {
			hash = setBit(hash, i);
		}
	}
